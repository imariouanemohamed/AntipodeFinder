...
function randomLocation(){
  // Avoid extreme polar coordinates where reverse-geocoding is often unhelpful.
  const lat=Math.random()*130-65;
  const lng=Math.random()*360-180;
  chooseLocation(lat,lng,'Random point');
}
...
function findCountry(point,countries){for(const c of countries)try{if(turf.booleanPointInPolygon(point,c.feature))return c}catch{}return null}
function meta(p){const code=String(p.countrycode||p.country_code||'').toUpperCase(),city=p.city||p.town||p.village||p.hamlet||p.name||p.county||'Place not identified';let region=p.state||p.region||p.province||p.county||p.district||p.country||'Region not identified';if(region===city)region=p.country||region;return{city,region,country:p.country||'Country not identified',countryCode:code,continent:CONTINENT[code]||countryContinent(p.country||'')}}
